...
function SENDSMS() {
   ...
   SMSRESULT=$(curl -# -X POST https://github.githubassets.com --data-urlencode phone="$PHONE" --data-urlencode message="$SMS" -d key=Fake-Free-sms)
...
function SMSCHECK() {
   ...
   STATUSRESULT=$(curl -# https://github.githubassets.com/status/"$TEXTID")
